                    ...
                    else // 登录成功
                    {
                        ...
                        if (responsejs.contains("offlinemsg"))
                        {
                            ...
                            for (auto &str : vec)
                            {
                                ...
                                {
                                    std::cout << "群消息[" << js["groupid"] << "]:" << js["time"].get<std::string>() << " [" << js["id"] << "]" << js["name"].get<std::string>() << " said: " << js["msg"].get<std::string>() << std::endl;
                                }
                            }
                        }

                        // 登录成功,启动接收线程,该线程只启动一次
...
void chat(int clientfd, std::string str)
{
    ...
    int friendid = stoi(str.substr(0, index));
...
void creategroup(int clientfd, std::string str)
{
    ...
    std::string groupdesc = str.substr(index + 1, str.length() - index - 1);
...
void groupchat(int clientfd, std::string str)
{
    ...
    int groupid = stoi(str.substr(0, index));
    std::string message = str.substr(index + 1, str.length() - index - 1);
    ...
    if (len == -1)
    {
        std::cerr << "group chat msg error -> " << buffer << std::endl;
    }
}
